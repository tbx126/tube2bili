...
## 0.2.0-rc.28 ASR 滚动结果时间轴（2026-09-26）

- 队列中的 Qwen Audio 3.0 ASR 缓存显示：同一 5 分钟片段产生 14–15 条相同起点、文本逐条累积的记录；旧流程把它们合并成 2–3 条长字幕，B 站拒绝其中一条（79014）。
- 启用词级时间戳，折叠累积快照并生成短字幕段；新增测试覆盖累积结果去重、字幕段时长/长度和 79014 错误分类。
- 完整测试 `python -m pytest -q`：97 项通过，另有两条第三方测试客户端弃用警告；`compileall` 与 `git diff --check` 通过。

## 0.2.0-rc.27 Qwen 长响应超时（2026-09-26）

- rc.26 脱敏事件确认：Qwen 小请求可用，但真实 ASR 英文翻译在 180 秒读超时。针对 Qwen 路由改为每批 8 条并延长到 300 秒，降低长提示超时风险，保留逐批检查点。
- 新增回归测试验证 17 条字幕拆为 8/8/1 批；完整测试 `python -m pytest -q`：93 项通过，另有两条第三方测试客户端弃用警告；`py_compile` 与 `git diff --check` 通过。

## 0.2.0-rc.26 Qwen 临时错误诊断与退避（2026-09-26）

- NAS 首条任务在语音识别成功后遇到翻译阶段通用错误；DashScope 小请求返回 HTTP 200，证明地址与凭证可用，但原日志未保留失败状态。现在对 Qwen 429/5xx、网络异常和无效 JSON 记录脱敏诊断，并保留任务进度后自动退避重试。
- 对 Qwen API 限流实施全局翻译阶段冷却，避免多个排队任务立刻重复调用；下载和非翻译阶段仍可继续。
- 新增覆盖 429 `Retry-After`、日志不泄露密钥和 worker 延后重试的测试；本地完整测试 92 项通过。

## 0.2.0-rc.25 YouTube TLS 断连与代理自动测速（2026-09-26）

- 队列视频请求因 Mihomo 当前手选节点出现 TLS `UNEXPECTED_EOF`；32 个订阅节点经 HTTPS 实测 20 个可用，切到 60ms 可用节点后 Google 和 YouTube 均返回 204。增加 AUTO 自动测速组后，32 个候选中 25 个通过实测，设为 PROXY 默认路由并每 5 分钟更新。
- TLS/SSL EOF 与握手失败现归类为网络故障；队列执行共享指数退避，代理变更会清除冷却并恢复受影响任务。
- 覆盖 TLS 分类、整队列退避和代理变更恢复的回归测试；完整测试结果见本版本交付说明。
