...
录音接口使用 `/services/audio/asr/transcription`，通过 `X-DashScope-Async: enable` 提交 `input.file_url`，轮询 `/tasks/{id}`，读取 `output.result.transcription_url`，将句子毫秒时间转换为字幕秒时间。普通 `qwen3-asr-flash` 不使用此协议，不能填入此路由。

音频每 10 分钟分段。任务编号保存在各分段检查点，暂停或查询超时后继续查询已有任务，结果缓存后不再请求。成功结果链接有效期为 24 小时；过期任务需处理失败后重试。提交请求成功但连接在返回任务编号前断开时，无法恢复未知编号，后续重试仍可能再次计费。

**当前上传采用千问官方临时存储（48 小时过期），适合联调验证。官方不推荐用于生产环境；长期无人值守部署仍需补充自有 OSS 支持及真实验证。** 不会将 NAS 暴露为公网文件服务器。模型 API Key 仅发送到配置的 API 地址，不随请求发往返回的存储上传、结果下载地址。
