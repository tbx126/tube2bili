...
function taskTable(tasks) {
  if (!tasks.length) return empty(query||filter?'没有匹配的任务':'队列已清空',query||filter?'尝试其他关键词，或切换状态筛选。':'粘贴 YouTube 链接，开始处理下一个视频。',query||filter?'clear-filter':'new-task',query||filter?'清除筛选':'添加视频链接');
  return `<div class="task-list">${tasks.map(t=>`<article class="task-card ${t.status==='running'?'is-running':''}"><div class="task-cover" aria-hidden="true">${t.bvid?'✓':t.status==='running'?'↗':'▷'}</div><div class="task-content"><div class="task-heading"><button class="task-title" data-action="detail" data-id="${t.id}">${esc(t.title||t.video_id)}</button>${badge(t.status)}</div><div class="task-meta"><span>${t.channel_id?'频道订阅':'手动导入'}</span><span>${esc(t.video_id)}</span><span>${date(t.created)}</span>${t.assets_deleted?'<span>文件已清理</span>':''}</div><div class="task-stage"><span>${t.status==='completed'?'发布与字幕已完成':stages[t.stage]}</span><strong>${Math.round(t.progress)}%</strong><div class="progress" role="progressbar" aria-label="任务进度" aria-valuenow="${Math.round(t.progress)}" aria-valuemin="0" aria-valuemax="100"><i style="width:${t.progress}%"></i></div></div>${t.error&&t.status!=='cancelled'?`<p class="task-error">${esc(t.error)}</p>`:''}</div><div class="task-actions">${['waiting','paused','failed','retrying'].includes(t.status)&&!t.assets_deleted?`<button data-action="task-action" data-id="${t.id}" data-command="resume">继续处理</button>`:''}${t.status==='running'&&t.stage!=='publish'?`<button data-action="task-action" data-id="${t.id}" data-command="pause">暂停</button>`:''}<button class="ghost" data-action="detail" data-id="${t.id}">查看详情</button><button class="ghost danger" data-action="delete-task" data-id="${t.id}" ${['running','reconcile'].includes(t.status)?'disabled title="请先停止执行或核对投稿"':''}>删除记录</button></div></article>`).join('')}</div>`;
}
...
function drawTasks() {
  $('#main').innerHTML=head('任务队列','从下载到发布，掌握每一步进展。')+`<div class="queue-summary" id="queue-summary"></div><section class="panel queue-panel"><div class="toolbar"><div class="search-field"><label for="task-search">搜索任务</label><input id="task-search" placeholder="输入标题或视频 ID" value="${esc(query)}"></div><div class="filter-field"><label for="task-filter">处理状态</label><select id="task-filter"><option value="">全部状态</option><option value="active" ${filter==='active'?'selected':''}>进行中</option><option value="attention" ${filter==='attention'?'selected':''}>需要处理</option>${Object.entries(states).map(([k,v])=>`<option value="${k}" ${filter===k?'selected':''}>${v}</option>`).join('')}</select></div><button class="ghost" data-action="refresh-tasks">刷新列表</button></div><div class="list-heading"><strong id="result-count"></strong><small>每 5 秒自动更新</small></div><div id="task-table"></div></section><p class="help queue-note">显示最近 300 条记录。删除记录保留本地文件和 B 站稿件；重新添加原链接可找回记录，避免重复投稿。</p>`;
  drawFiltered();
}
function drawFiltered(){
  const selected=overview.tasks.filter(t=>(!filter||(filter==='active'?isActive(t):filter==='attention'?needsAttention(t):t.status===filter))&&(`${t.title} ${t.video_id}`.toLowerCase().includes(query.toLowerCase())));
  $('#task-table').innerHTML=taskTable(selected);
  if($('#result-count'))$('#result-count').textContent=`${selected.length} 条记录`;
  if($('#queue-summary'))$('#queue-summary').innerHTML=[['active','进行中',overview.tasks.filter(isActive).length,'正在执行或等待执行'],['attention','需要处理',overview.tasks.filter(needsAttention).length,'检查配置或核对投稿'],['completed','已完成',overview.tasks.filter(t=>t.status==='completed').length,'视频与字幕均已确认'],['','全部记录',overview.tasks.length,'当前队列中的任务']].map(([value,label,count,hint])=>`<button class="queue-stat ${filter===value?'selected':''}" data-action="filter-tasks" data-filter="${value}" aria-pressed="${filter===value}"><span>${label}</span><strong>${count}</strong><small>${hint}</small></button>`).join('');
}
...
async function drawSettings(){
  ...
  $('#main').innerHTML=head('服务设置','连接你的模型服务、投稿账号和通知渠道。',null)+`<form id="settings-form" class="settings-layout">
    ...
    ${['translation','transcription'].map(p=>`<section class="panel"><div class="panel-head"><h2>${p==='translation'?'字幕与文案翻译':'无字幕视频 · 语音识别'}</h2><small>千问 / OpenAI 兼容</small></div><div class="panel-body"><p class="help">${p==='translation'?'支持 /chat/completions，按字幕段落翻译并保存进度。':'推荐千问音频直传：每 5 分钟分段，无需 OSS。Filetrans 使用临时存储，适合联调；其他服务需支持 verbose_json 时间轴。'}</p><div class="route-grid">${routeForm(p,'primary',s[p].primary)}${routeForm(p,'fallback',s[p].fallback)}</div><label class="check-label" style="margin-top:18px;margin-bottom:0"><input name="${p}.fallback_enabled" type="checkbox" ${s[p].fallback_enabled?'checked':''}>允许主服务失败时调用备用服务（可能产生费用）</label></div></section>`).join('')}
...
document.addEventListener('click',async e=>{
  ...
  try{
    ...
    if(action==='refresh-tasks'){button.disabled=true;try{await refresh();toast('队列已更新');}finally{button.disabled=false;}return;}
    if(action==='delete-task'){
      const task=overview.tasks.find(t=>t.id===id);
      modal('删除任务记录',`<p>将「${esc(task?.title||task?.video_id||id)}」从队列中移除。</p><div class="delete-note">尚未执行的任务会停止排队。本地视频、字幕、已发布稿件及去重信息均保留。需要清理磁盘时，请在任务详情中单独删除文件。</div><div class="row confirm-actions"><button class="ghost" data-action="close" autofocus>保留记录</button><button class="danger-solid" data-action="confirm-delete-task" data-id="${id}">删除记录</button></div>`);return;
    }
    if(action==='confirm-delete-task'){button.disabled=true;try{await api(`/tasks/${id}`,'DELETE');$('#modal').close();await refresh();toast('记录已从队列移除，本地文件已保留');}finally{button.disabled=false;}return;}
    ...
    if(action==='qwen-preset'){
      ...
      const values={name:translation?'Qwen 翻译':'Qwen 录音识别',protocol:translation?'qwen':'qwen_audio',base_url:'https://dashscope.aliyuncs.com/'+(translation?'compatible-mode/v1':'api/v1'),model:translation?'qwen-plus':'qwen-audio-3.0-asr-flash'};
...
document.addEventListener('submit',async e=>{
  ...
  try{
    ...
    if(form.id==='task-form'){const result=await api('/tasks','POST',{url:data.get('url')});await refresh();await showDetail(result.id);toast('已打开任务；已有视频保留原处理状态');}
