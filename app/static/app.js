...
async function drawSettings(){
  ...
  $('#main').innerHTML=head('服务设置','连接你的模型服务、投稿账号和通知渠道。',null)+`<form id="settings-form" class="settings-layout">
    ...
    ${['translation','transcription'].map(p=>`<section class="panel"><div class="panel-head"><h2>${p==='translation'?'字幕与文案翻译':'无字幕视频 · 语音识别'}</h2><small>OpenAI 兼容协议</small></div><div class="panel-body"><p class="help">${p==='translation'?'支持 /chat/completions，按字幕段落翻译并保存进度。':'支持 /audio/transcriptions，必须返回 verbose_json 分段时间轴。音频每 10 分钟分段上传。'}</p><div class="route-grid">${routeForm(p,'primary',s[p].primary)}${routeForm(p,'fallback',s[p].fallback)}</div><label class="check-label" style="margin-top:18px;margin-bottom:0"><input name="${p}.fallback_enabled" type="checkbox" ${s[p].fallback_enabled?'checked':''}>允许主服务失败时调用备用服务（可能产生费用）</label></div></section>`).join('')}
    ...
    <section class="panel"><div class="panel-head"><h2>平台登录凭证</h2><small>仅保存在 NAS 数据目录</small></div><div class="panel-body"><div class="row between"><div><strong>Bilibili</strong><p class="help">${s.bilibili_configured?'已导入登录文件（有效性在投稿时检查）':'尚未导入 biliup 登录文件'}</p></div><button type="button" data-action="credentials" data-provider="bilibili">导入 cookies.json</button></div><div class="row between"><div><strong>YouTube</strong><p class="help">${s.youtube_configured?'已配置 cookies.txt':'可选：遇到登录限制时导入 Netscape cookies.txt'}</p></div><button type="button" data-action="credentials" data-provider="youtube">导入 cookies.txt</button></div><p class="help">也可以在 NAS 终端执行：<code>docker compose exec app biliup -u /data/cookies.json login</code></p></div></section>
